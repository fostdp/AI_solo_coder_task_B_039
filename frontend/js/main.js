class App {
    ...
    async init() {
        this.tankScene = new TankScene('three-canvas');
        this.tankScene.onSensorClick = (sensorData) => this.handleSensorClick(sensorData);
    ...
    updateTankVisualization() {
        ...
        this.tankScene.updateTemperatureData(this.tankData.layerTemps || []);
        this.tankScene.updateDensityData(this.tankData.densities || []);
        this.tankScene.drawOverlay(this.tankData);
    }
...
window.addEventListener('resize', () => {
    if (window.app) {
        if (window.app.tempChart) window.app.tempChart.resize();
        if (window.app.densityChart) window.app.densityChart.resize();
        if (window.app.riskChart) window.app.riskChart.resize();
    }
});
